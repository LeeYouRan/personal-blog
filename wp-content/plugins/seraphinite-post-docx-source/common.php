...
{
	return( \seraph_pds\Ui::Button( $content, $primary, $nameId, $classesEx, $type, $attrs ) );
}
...
function GetPostsAvailableCategories( $postTypes = NULL )
{
	...
	return( Wp::GetPostsAvailableTaxonomies( Wp::POST_TAXONOMY_TYPE_CATEGORY, $postTypes ) );
}
...
function getUrl( $url, $body = false )
{
	...
	$prms = array( 'sslverify' => false, 'timeout' => 15 );

	$requestRes = Wp::RemoteGet( $url, $prms );

	if( is_wp_error( $requestRes ) )
		return( array( 'hr' => Gen::E_FAIL ) );
...
{
	$requestRes = Wp::RemoteGet( $url, array( 'sslverify' => false, 'timeout' => 15 ) );
	...
	_dublHdr( $requestRes, 'content-transfer-encoding' );
	_dublHdr( $requestRes, 'content-type' );
	_dublHdr( $requestRes, 'content-length' );
