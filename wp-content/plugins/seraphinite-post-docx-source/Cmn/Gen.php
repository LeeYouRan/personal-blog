...
class Gen
{
	...
	static function GetFileDir( $filepath, $levels = 1 )
	{
...
class Net
{
	...
	static function Url2Uri( $url, $siteUrlRelative = false )
	{
		...
		$siteUrl = self::GetUrlWithoutProto( Gen::SetLastSlash( $siteUrlRelative === 'home' ? home_url() : get_site_url(), false ) );
		$url = self::GetUrlWithoutProto( $url );
...
class HtmlNd
{
	...
	static function FindBy( $nd, $func, $data = null )
	{
		if( !$nd )
			return( null );
		...
		for( $i = 0; $i < $children -> length; $i++ )
		{
			$ndRes = self::FindBy( $children -> item( $i ), $func, $data );
			if( $ndRes )
				return( $ndRes );
		}
	...
	{
		return( self::FindBy( $nd, function( $nd, $tag ) { return( $nd -> nodeName == $tag ); }, $tag ) );
	}
	...
	static function SetTag( $nd, $name )
	{
		if( !$nd )
			return( $nd );
...
class Wp
{
	...
	{
		$fileNamePathTmp = tempnam( sys_get_temp_dir(), substr( 'pds', 0, 3 ) );
	...
	static function GetLocString( $id, $number = null, $domain = null )
	{
		$ctx = null;
		if( is_array( $id ) )
		{
	...
	static function Loc_ScriptLoadEx( $handle, $domain, $ns, $pathAbsRoot )
	{
		...
		return( \wp_localize_script( $handle, '_' . $handle . '_scriptLocData', array( 'l10n_print_after' => 'jQuery(document).on("ready",function($){var data = ' . $translations . ';seraph_pds.Wp.Loc.SetData(data.locale_data.messages,"' . $domain . '");})' ) ) );
	}
	...
	static function Loc_ScriptLoad( $handle, $domain, $locSubPath )
	{
		...
		return( self::Loc_ScriptLoadEx( $handle, $domain, 'seraph_pds', $pathAbsRoot . '/seraphinite-post-docx-source/' . $locSubPath ) );
	}
	...
	static function Loc_LoadEx( $subSystemIds = null, $domain, $pathAbsRoot )
	{
		self::_Loc_DomainExpand( $domain, $domainFile );
	...
	static function Loc_Load( $subSystemIds = null, $domain, $locSubPath, array $addFiles = array() )
	{
		$subSystemIds = self::_Loc_LoadPrepareSubSystemIds( $subSystemIds );
		if( !count( $subSystemIds ) )
			return( false );
